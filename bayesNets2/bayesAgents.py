...
def fillHouseCPT(bayesNet, gameState):
    foodHouseFactor = bn.Factor(
        [FOOD_HOUSE_VAR], [X_POS_VAR, Y_POS_VAR], bayesNet.variableDomainsDict())
    ...
    ghostHouseFactor = bn.Factor(
        [GHOST_HOUSE_VAR], [X_POS_VAR, Y_POS_VAR], bayesNet.variableDomainsDict())
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                ...
                wall = assignment[obsVar]

                adjacentHousePos = getAdjacentHousePos(gameState, obsPos)
                adjacentHouseVal = getAdjacentHouseVal(gameState, adjacentHousePos)
                prob = getObsProb(adjacentHouseVal, foodHouseVal, ghostHouseVal, wall)
                obsFactor.setProbability(assignment, prob)
            bayesNet.setCPT(obsVar, obsFactor)

def getAdjacentHousePos(gameState, obsPos):
    houseObsDis = float("inf")
    adjacentHousePos = None
    for housePos in gameState.getPossibleHouses():
        distance = util.manhattanDistance(housePos, obsPos)
        if distance < houseObsDis:
            houseObsDis = distance
            adjacentHousePos = housePos
    return adjacentHousePos

def getAdjacentHouseVal(gameState, adjacentHousePos):
    bottomLeftPos, topLeftPos, bottomRightPos, topRightPos = gameState.getPossibleHouses()
...
def getObsProb(adjacentHouseVal, foodHouseVal, ghostHouseVal, wall):
    ...
    elif adjacentHouseVal == ghostHouseVal:
        if wall == RED_OBS_VAL:
            prob = PROB_GHOST_RED
        elif wall == BLUE_OBS_VAL:
